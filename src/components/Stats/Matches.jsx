...
async function getMatches(teamId) {
  ...
  const apiUrl = 'https://www.robotevents.com/api/v2/teams/${teamId}/matches?event%5B%5D=${eventId}';
  ...
  return fetch(apiUrl, {
    ...
    .then(data => {
      if (data.data && data.data.length > 0) {
        ...
        console.log(`matches for Team ${teamId}`);
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const matchNumberGlobal = await getMatches(teamID);
          setMatchNum(matchNumberGlobal);
    ...
    <div className={'border-2 rounded-xl border-gray-500/50 p-4 md:p-8 backdrop-blur-[10px] '}>
        <table className='w-full md:w-[60vw]'>
            ...
            <tr className='text-white font-rubik text-lg md:text-xl text-left'>
                <th className="pr-2 md:pr-4">MATCH NUM</th>
                <th className="pr-2 md:pr-4">RED ALLIANCE</th>
                <th className="pr-2 md:pr-4">RED SCORE</th>
                <th className="pr-2 md:pr-4">BLUE ALLIANCE</th>
                <th className="pr-2 md:pr-4">BLUE SCORE</th>
            </tr>
            </thead>
            <tbody className='text-regular text-white'>
            <tr>
                <td className='pr-2 md:pr-4 pt-2 md:pt-4'>{matchNum}</td>
                <td className='pr-2 md:pr-4 pt-2 md:pt-4'>i</td>
                <td className='pr-2 md:pr-4 pt-2 md:pt-4'>ui</td>
            </tr>
            </tbody>
        </table>
        <h4 className='w-full text-white text-center text-lg md:text-xl pt-2 md:pt-8'>
            {rings ? (
            rings.map((award, index) => (
                <th key={index}>{award}</th>
            ))
            ) : (
            'Loading awards...'
            )}
